...
serviceCall = function (query, options) {
    ...
    var version = (query.uri.segments.count || 
                   query.uri.segments.options.inlinecount || 
                   query.uri.segments.options.select !== undefined) ? '2.0' : '1.0';
    var req = {
        headers: {
    ...
    if (settings.etag !== null) {
        req.headers['If-Match'] = settings.etag;
    }

    // call users beforeRequest if specified
    var beforeRequestHandler = (typeof settings.beforeRequestHandler === 'function')
        ? settings.beforeRequestHandler
        : function ( req, cb ) { cb(); };

    beforeRequestHandler( req, function ( ) {
