...
odataUri = function (uriSegments) {
    var that,
        ...
        // use built-in String.trim function if one is available, otherwise use jQuery.trim.
        trim = function ( str ) {
            return (str || '').trim();
        },
    ...
    that.isLocalService = false;// that.segments.root !== null &&
...
serviceCall = function (query, options) {
    ...
    if (typeof settings.beforeSend === 'function') {
        ...
        settings.beforeSend(req, function ( ) {
            request(req, function (err, res, body) {
                if (err || ~~(res.statusCode / 100) !== 2){
                    settings.error(err || res.statusCode);
                } else {
                    console.log(body);
                    settings.success(JSON.parse(body));
                }
                settings.complete && settings.complete(err, res, body);
            });
        });
    } else {
        request(req, function (err, res, body) {
